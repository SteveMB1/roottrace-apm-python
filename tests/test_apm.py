...
class ProfileSettingsTests(unittest.TestCase):
    """The clamp that bounds what the server can ask this process to do.

    This is the security boundary that matters: the value becomes a sampling
    loop inside a customer's production process, so the SDK bounds it itself
    rather than trusting the control plane to have done so.
    """

    def test_profiling_stays_off_until_the_server_turns_it_on(self):
        self.assertFalse(apm_mod._clamp_profile_settings(None)["profiling_enabled"])
        self.assertFalse(apm_mod._clamp_profile_settings({})["profiling_enabled"])

    def test_a_ten_kilohertz_sample_rate_comes_back_bounded(self):
        clamped = apm_mod._clamp_profile_settings({"sample_rate_hz": 10_000})
        self.assertEqual(clamped["sample_rate_hz"], 200)

    def test_a_rate_below_the_floor_is_raised(self):
        self.assertEqual(
            apm_mod._clamp_profile_settings({"sample_rate_hz": 0})["sample_rate_hz"], 10
        )

    def test_a_non_numeric_setting_falls_back_to_the_default(self):
        self.assertEqual(
            apm_mod._clamp_profile_settings({"sample_rate_hz": "fast"})["sample_rate_hz"], 100
        )

    def test_every_published_bound_is_enforced(self):
        wild = {key: 10 ** 9 for key in apm_mod.PROFILE_BOUNDS}
        clamped = apm_mod._clamp_profile_settings(wild)
        for key, (low, high) in apm_mod.PROFILE_BOUNDS.items():
            self.assertGreaterEqual(clamped[key], low, key)
            self.assertLessEqual(clamped[key], high, key)


class FrameNameTests(unittest.TestCase):
    def test_a_frame_is_module_and_function_without_a_line(self):
        """A line number would give a function a new identity every time an
        edit shifted it down, breaking diffs across a deploy."""
        code = (lambda: None).__code__
        name = apm_mod._frame_name(code)
        self.assertEqual(name, "test_apm.<lambda>")
        self.assertNotIn(":", name)


class ProfilerTests(unittest.TestCase):
    def setUp(self):
        self.settings = apm_mod._clamp_profile_settings(
            {"profiling_enabled": True, "sample_rate_hz": 200}
        )
        self.profiler = apm_mod._Profiler(None, self.settings)

    def test_a_drain_with_nothing_sampled_is_none_not_an_empty_profile(self):
        self.assertIsNone(self.profiler.drain())

    def test_the_hot_function_owns_its_self_time(self):
        """The claim the feature rests on: burn time in a named function and
        that function is the leaf holding it.

        Deliberately not "the widest stack in the profile" -- this is a
        WALL-CLOCK profiler across every thread, so an idle pool thread parked
        in a wait is exactly as wide as a thread spinning. That is the correct
        measurement and the reason the profile is labelled `wall` everywhere.
        """
        self.profiler.start()
        try:
        ...
        self.assertEqual(profile["profile_type"], "wall")
        self.assertEqual(profile["value_unit"], "nanoseconds")
        mine = [
            stack for stack in profile["stacks"]
            if stack["frames"][-1].endswith("test_the_hot_function_owns_its_self_time")
        ]
        self.assertEqual(len(mine), 1, [s["frames"][-1] for s in profile["stacks"]])
        # Effectively the whole window, since this thread never left the loop.
        self.assertGreater(mine[0]["value"], 0.7 * profile["duration_ms"] * 1e6)

    def test_samples_are_weighted_by_the_period_actually_achieved(self):
        """Under the GIL a sampler competing with a hot loop is scheduled late
        and takes fewer passes than its rate implies. Weighting by the nominal
        period would understate every duration by exactly the fraction of
        ticks that were missed, so the totals would not add up to the window."""
        self.profiler.start()
        try:
            deadline = time.monotonic() + 0.4
            x = 0
        ...
        time.sleep(0.05)
        profile = self.profiler.drain()
        window_ns = profile["duration_ms"] * 1e6
        # One thread sampled throughout, so its attributed time is the window.
        widest = max(stack["value"] for stack in profile["stacks"])
        self.assertAlmostEqual(widest / window_ns, 1.0, delta=0.15)

    def test_the_profiler_does_not_profile_itself(self):
        self.profiler.start()
        time.sleep(0.15)
        self.profiler.stop()
        profile = self.profiler.drain()
        for stack in profile["stacks"]:
            self.assertNotIn("roottrace_apm._Profiler._sample", stack["frames"])

    def test_distinct_stacks_are_capped_without_losing_the_profile(self):
        capped = apm_mod._Profiler(None, {**self.settings, "max_stacks": 500})
        capped._counts = {("a",): 1}
        capped._settings = {**capped._settings, "max_stacks": 1}
        capped._rounds = 1
        capped._sample()  # every new stack past the cap is dropped, not stored
        self.assertLessEqual(len(capped._counts), 1)


class ProfileConfigTests(unittest.TestCase):
    """Fail-closed: an SDK that defaults to on when the control plane is
    unreachable is the wrong failure direction for code running inside
    somebody else's process."""

    def setUp(self):
        self.apm = apm_mod.Apm(service="svc", token="rtc_test", api_url="http://127.0.0.1:1/api")

    def tearDown(self):
        if self.apm._profiler is not None:
            self.apm._profiler.stop()

    def _allow_next_poll(self):
        """Make the throttle treat the next call as due.

        -inf, never 0. time.monotonic() is uptime on Linux, so on a CI runner
        a few seconds into its life 0 reads as *recent* and the poll is
        skipped -- which is the same clock mistake the SDK itself shipped, and
        it makes these tests pass or fail on how long the host has been up.
        """
        self.apm._profile_config_at = float("-inf")

    def test_an_unreachable_control_plane_leaves_profiling_off(self):
        with mock.patch.object(self.apm, "_get", side_effect=OSError("no route")):
            self.apm._refresh_profile_config()
        self.assertFalse(self.apm.profiling)

    def test_a_running_profiler_survives_a_transient_fetch_failure(self):
        with mock.patch.object(
            self.apm, "_get", return_value={"profiling_enabled": True, "sample_rate_hz": 50}
        ):
            self.apm._refresh_profile_config()
        self.assertTrue(self.apm.profiling)
        self._allow_next_poll()
        with mock.patch.object(self.apm, "_get", side_effect=OSError("blip")):
            self.apm._refresh_profile_config()
        self.assertTrue(self.apm.profiling)

    def test_turning_it_off_in_the_ui_stops_the_sampler(self):
        with mock.patch.object(self.apm, "_get", return_value={"profiling_enabled": True}):
            self.apm._refresh_profile_config()
        self.assertTrue(self.apm.profiling)
        self._allow_next_poll()
        with mock.patch.object(self.apm, "_get", return_value={"profiling_enabled": False}):
            self.apm._refresh_profile_config()
        self.assertFalse(self.apm.profiling)

    def test_a_server_asking_for_ten_kilohertz_gets_the_clamped_rate(self):
        with mock.patch.object(
            self.apm, "_get", return_value={"profiling_enabled": True, "sample_rate_hz": 10_000}
        ):
            self.apm._refresh_profile_config()
        self.assertEqual(self.apm._profile_config["sample_rate_hz"], 200)
        self.assertEqual(self.apm._profiler._settings["sample_rate_hz"], 200)

    def test_the_first_poll_runs_on_a_freshly_booted_host(self):
        """time.monotonic() is uptime on Linux, so a container that starts
        seconds after boot reads a clock in single digits. The interval guard
        must treat "never fetched" as due rather than comparing against a zero
        that a young host has not yet passed -- otherwise profiling stays off
        for the first minute on exactly the machines that restart most.

        This is invisible on a long-lived workstation, which is why it reached
        CI: the developer's clock was 26,000 seconds in.
        """
        with mock.patch("time.monotonic", return_value=5.0):
            apm = apm_mod.Apm(service="svc", token="t", api_url="http://x/api")
            with mock.patch.object(
                apm, "_get", return_value={"profiling_enabled": True}
            ):
                apm._refresh_profile_config()
            self.addCleanup(lambda: apm._profiler and apm._profiler.stop())
            self.assertTrue(apm.profiling)
            self.assertIsNotNone(apm._profile_config)

    def test_a_profile_is_not_uploaded_before_its_interval_elapses(self):
        """upload_interval_seconds is a control in the RootTrace UI, and the
        metric flush cadence is a different number entirely -- 30s by default
        and settable to 5. Uploading per metric flush would cut a 60s profile
        into slivers and make the setting a lie."""
        with mock.patch.object(
            self.apm,
            "_get",
            return_value={"profiling_enabled": True, "upload_interval_seconds": 60},
        ):
            self.apm._refresh_profile_config()
        self.assertTrue(self.apm.profiling)
        with mock.patch.object(self.apm._profiler, "drain") as drain:
            self.apm._flush_profile()
        drain.assert_not_called()

    def test_a_profile_is_uploaded_once_its_interval_has_elapsed(self):
        with mock.patch.object(
            self.apm,
            "_get",
            return_value={"profiling_enabled": True, "upload_interval_seconds": 60},
        ...
        self.apm._profile_upload_at -= 60
        with mock.patch.object(self.apm._profiler, "drain", return_value=None) as drain:
            self.apm._flush_profile()
        drain.assert_called_once()

    def test_the_upload_window_is_the_servers_number_not_the_flush_cadence(self):
        with mock.patch.object(
            self.apm,
            "_get",
            return_value={"profiling_enabled": True, "upload_interval_seconds": 300},
        ):
            self.apm._refresh_profile_config()
        # A whole metric flush interval later is still inside a 300s window.
        self.apm._profile_upload_at -= 30
        with mock.patch.object(self.apm._profiler, "drain") as drain:
            self.apm._flush_profile()
        drain.assert_not_called()

    def test_the_second_poll_inside_the_interval_is_skipped(self):
        """The guard still has to throttle once a fetch has happened, or every
        flush would open a connection to the control plane."""
        with mock.patch("time.monotonic", return_value=5.0):
            apm = apm_mod.Apm(service="svc", token="t", api_url="http://x/api")
            with mock.patch.object(
                apm, "_get", return_value={"profiling_enabled": False}
            ) as fetch:
                apm._refresh_profile_config()
                apm._refresh_profile_config()
            self.assertEqual(fetch.call_count, 1)

    def test_the_config_url_names_the_service_and_nothing_else(self):
        """Scope comes from the token on the server side; the SDK cannot ask
        for another environment's settings even if it wanted to."""
        apm = apm_mod.Apm(service="my svc", token="t", api_url="http://x/api")
        self.assertEqual(apm.config_url, "http://x/api/apm/config?service=my%20svc")



class SecuritySinkTests(unittest.TestCase):
    """The deterministic half of intrusion detection.

    A sampling profiler misses a shell that runs for five milliseconds. These
    call sites do not, and because they fire inside a transaction the report
    carries the request that reached them -- which is the whole traceback.
    """

    def setUp(self):
        apm_mod._instrument_security_sinks()
        with apm_mod._security_lock:
            del apm_mod._security_events[:]
    ...
    def _events(self):
        with apm_mod._security_lock:
            return list(apm_mod._security_events)

    def test_spawning_a_process_is_reported(self):
        import subprocess

        subprocess.run(["/bin/echo", "hello"], capture_output=True)
        events = self._events()
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]["kind"], "process_spawn")
        self.assertEqual(events[0]["target"], "/bin/echo")

    def test_the_arguments_are_never_recorded(self):
        """argv is the attacker's payload. The executable identifies the
        behaviour; the arguments would be the exfiltration."""
        import subprocess

        subprocess.run(["/bin/echo", "s3cret-token-value"], capture_output=True)
        blob = repr(self._events())
        self.assertNotIn("s3cret-token-value", blob)

    def test_the_wrapped_call_still_works(self):
        """Instrumentation that breaks the application it watches is worse
    ...
    def test_unpickling_is_reported(self):
        import pickle

        self.assertEqual(pickle.loads(pickle.dumps({"a": 1})), {"a": 1})
        kinds = [event["kind"] for event in self._events()]
        self.assertIn("deserialization", kinds)

    def test_a_report_carries_the_request_that_reached_it(self):
        with apm_mod.transaction("POST /upload") as tx:
            tx.set_http(method="POST", path="/upload?x=1", status_code=200)
            apm_mod._report_sink("process_spawn", "/bin/sh")
        event = self._events()[-1]
        self.assertEqual(event["transaction"], "POST /upload")
        self.assertEqual(event["method"], "POST")
        self.assertEqual(event["path"], "/upload?x=1")
        self.assertIsNotNone(event["trace_id"])

    def test_the_buffer_cannot_grow_without_bound(self):
        """A process being driven by an attacker produces these faster than
        any flush interval; an unbounded buffer inside the victim is its own
        denial of service."""
        for _ in range(apm_mod.SECURITY_MAX_EVENTS + 50):
            apm_mod._report_sink("process_spawn", "/bin/sh")
        self.assertEqual(len(self._events()), apm_mod.SECURITY_MAX_EVENTS)

    def test_reporting_never_raises_into_the_application(self):
        # An unserialisable target must not propagate out of a request handler.
        class Hostile:
            def __str__(self):
                raise RuntimeError("nope")

        apm_mod._report_sink("process_spawn", Hostile())  # must not raise
